        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                if "StringDtype" in error_str or "dtype" in error_str.lower():
                    # The issue is likely with pandas version mismatch
                    # Try to load with compatibility mode
                    try:
                        # Method 1: Try with mmap_mode=None
                        pipeline = joblib.load(MODEL_FILE, mmap_mode=None)
                    except:
                        # Method 2: Try loading with pickle directly and handle StringDtype
                        import io
                        import pandas as pd
                        
                        # Monkey patch to handle StringDtype if needed
                        original_string_dtype = getattr(pd, 'StringDtype', None)
                        
                        # Try to create a compatibility layer
                        class CompatibleStringDtype:
                            def __init__(self, *args, **kwargs):
                                pass
                        
                        # Temporarily replace StringDtype if it causes issues
                        try:
                            ...
                            raise ValueError(
                                "Model loading failed due to pandas StringDtype compatibility. "
                                "This usually happens when the model was saved with pandas 1.3+ "
                                "and you're loading with a different version. "
                                "Solution: Retrain the model with the same pandas version, "
                                "or ensure pandas>=1.3.0,<2.0.0 is installed."
                            )
                else:
                    raise
